...
export const CollapsibleDisplay: React.FC<CollapsibleDisplayProps> = ({
  ...
  return (
    <div className={`rounded border border-gray-200/70 dark:border-gray-700/60 border-l-2 ${borderColor} bg-white/40 dark:bg-gray-950/20 px-2 py-1 my-1 ${className}`}>
      <CollapsibleSection
        title={title}
        toolName={toolName}
        open={defaultOpen}
        ...
        wrapTitle={wrapTitle}
        detailsId={toolId ? `tool-result-${toolId}` : undefined}
      >
        {children}
