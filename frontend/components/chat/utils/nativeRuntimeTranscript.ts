...
function getItemId(data: Record<string, unknown>): unknown {
  ...
  return data.itemId ?? item.id ?? item.call_id ?? item.callId ?? item.toolCallId;
}
...
export function reduceNativeRuntimeEvent(
  ...
  if (existingIndex >= 0) {
    ...
    // Incrementally merge tool payload so output deltas keep the existing
    // visible command / tool input.  Without this guard an output-only delta
    // would overwrite the toolInput with undefined when it lacks a command field.
    const mergedToolInput =
      toolPayload.toolInput !== undefined && toolPayload.toolInput !== null && toolPayload.toolInput !== ''
        ? toolPayload.toolInput
        : existing.toolInput;
    const mergedToolResult =
      toolPayload.toolResult !== undefined && toolPayload.toolResult !== null
        ? toolPayload.toolResult
        : existing.toolResult;
    const mergedToolName =
      toolPayload.toolName !== undefined && toolPayload.toolName !== null && toolPayload.toolName !== ''
        ? toolPayload.toolName
        : existing.toolName;
