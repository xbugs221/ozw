 ...
 */
function isNativeLiveMessage(message: ChatMessage): boolean {
  /**
   * Native SDK streams use provider-live source names while older Claude code
   * used provider-realtime; both represent unpersisted rows owned by the view.
   */
...
export function useChatSessionState({
  ...
  /** Cursor for content-aware delta refresh on co-backed sessions. */
  const appendCursorRef = useRef<string | null>(null);
  ...
  const loadSessionMessages = useCallback(
    async (projectName: string, sessionId: string, loadMore = false, provider: string = 'codex', projectPath: string = '') => {
      ...
      try {
        ...
        if (isInitialLoad && result.appendCursor) {
          appendCursorRef.current = result.appendCursor;
        }
  ...
  const appendLatestSessionMessages = useCallback(async () => {
    ...
    try {
      ...
      // For co-backed sessions, use content-aware cursor refresh to detect
      // same-message content growth (e.g. Pi streaming).  Non-co sessions
      // continue using afterLine (JSONL is append-only).
      const result = isCoSession
        ? await fetchSessionMessages(
            ...
            null,  // afterLine: not used for co cursor refresh
            appendCursorRef.current,  // afterCursor
          )
        : await fetchSessionMessages(
            sessionProjectName,
            sessionLoadId,
            ...
            0,
            resolveSessionProvider(curProject, curSession) || 'codex',
            knownTotal,  // afterLine: skip known rows
            null,  // afterCursor: not used for non-co
          );
      ...
      // Update the cursor for next refresh.
      if (result.appendCursor) {
        appendCursorRef.current = result.appendCursor;
      }
