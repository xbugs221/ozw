...
import { routeChatRealtimeEvent, applyRealtimeSessionEvent } from './chatRealtimeEventRouter';
import { appendStreamingChunk, finalizeStreamingMessage } from './streamingMessageController';
 ...
 */
async function reloadCodexSessionMessages({
  selectedProject,
  selectedSession,
  sessionId,
  loadSessionMessages,
  ...
  // For cN route sessions, preserve the route id so the server can merge
  // provider JSONL with active-turn overlays instead of returning stale history.
  const resolvedSessionId = getSessionLoadId(selectedSession || null) || sessionId;
...
export function useChatRealtimeHandlers({
  ...
  useEffect(() => {
    ...
    pendingMessages.forEach(({ sequence, message: latestMessage }) => {
      const realtimeRoute = routeChatRealtimeEvent(latestMessage);
      void realtimeRoute;
      void applyRealtimeSessionEvent(selectedSession, latestMessage);
        ...
        if (structuredMessageData && Array.isArray(structuredMessageData.content)) {
          ...
          structuredMessageData.content.forEach((part: any) => {
            if (part.type === 'tool_use') {
              ...
              // Check if this is a subagent tool across Claude, Codex, and Pi payloads.
      ...
      case 'codex-response': {
        ...
        const CODEX_LIVE_ITEM_TYPES = ['agent_message', 'reasoning', 'thinking', 'command_execution', 'tool_call', 'tool_result', 'file_change', 'mcp_tool_call', 'function_call', 'custom_tool_call', 'function_call_output', 'update', 'error'];
