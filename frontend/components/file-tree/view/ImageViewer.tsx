...
export default function ImageViewer({ file, onClose }: ImageViewerProps) {
  const imageQuery = new URLSearchParams({ path: file.path });
  if (file.projectPath) {
    imageQuery.set('projectPath', file.projectPath);
  }
  const imagePath = `/api/projects/${file.projectName}/files/content?${imageQuery.toString()}`;
