...
### Highlights
- 使用“轨道”品牌图标，统一浏览器标签与 PWA 安装图标。
- 修复 GPT-6 会话的价值显示。

### Quality
- PR 检查按改动范围运行，减少无关检查的等待时间。

## v1.4.20 - 2026-09-23

### Distribution
- 修复无可选依赖的安装验收，解除自动发布门禁。
...
### Distribution
- npm 包名改为 `@xbugs221/ozw`，为按包名安装和标签自动发布做准备。

## v1.4.17 - 2026-09-23

### Fixes
- 同一轮对话的正文前后过程消息共用一个折叠栏，避免重复显示耗时。

## v1.4.16 - 2026-09-23

### Quality
- Windows 发行验收完成所有检查后显式退出，避免 ConPTY 遗留句柄阻塞发版。
- 安装矩阵增加 12 分钟超时，异常时给出明确失败结果。

## v1.4.15 - 2026-09-23

### Highlights
- 同一对话获得路由编号后继续复用原有 tmux window。
- 项目主页提供返回首页入口。
- 推送版本标签后，流水线自动验证并发布可直接安装的 GitHub 发行包。

### Quality
- tmux 聚焦测试等待客户端真正就绪，避免慢速 runner 中误判。
- 主分支要求通过 PR 和两项 CI 检查，合并历史保持线性。

## v1.4.13 - 2026-09-21

### Highlights
- 项目首页默认折叠自动工作流，减少首屏干扰。
- 左侧项目导航只展示近三天活跃项目，旧项目集中到非活跃分组。
- 渲染 Tab 会读取当前聚焦的 tmux window，跟随同一项目的会话切换。

### Changes
- 增加 tmux pane 与 Provider 会话的解析和映射。
- 保留项目索引活动时间，支持轻量列表准确分组。

### Quality
- 补充 tmux 聚焦窗口、项目活动边界和项目索引回归测试。
