...
describe('no-stale-test-contract', () => {
  ...
  it('package.json 包含委托 test:full 的默认全量入口', () => {
    const pkg = JSON.parse(readFileSync(resolve(REPO_ROOT, 'package.json'), 'utf8'));
    ok(typeof pkg.scripts?.test === 'string', 'package.json 缺少 test 脚本');
    ok(pkg.scripts.test === 'pnpm run test:full', 'test 脚本应委托 test:full');
    ok(pkg.scripts['test:full']?.includes('typecheck'), 'test:full 应包含 typecheck');
    ok(pkg.scripts['test:full']?.includes('test:node'), 'test:full 应包含完整 Node 回归');
    ok(pkg.scripts['test:full']?.includes('test:browser:full'), 'test:full 应包含完整浏览器回归');
  });
});
