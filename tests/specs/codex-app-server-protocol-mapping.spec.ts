...
const PROTOCOL_LOG_PATH = path.join(
  process.cwd(),
  'test-results/oz-113-codex-app-server-runtime/protocol.log',
);
const protocolEvidence: string[] = [];

/**
 * 记录协议测试覆盖到的业务行为，供 acceptance required_evidence 复现。
 */
function recordProtocolEvidence(message: string): void {
  protocolEvidence.push(message);
}

/**
 * 写入 Codex app-server protocol runtime evidence。
 */
async function writeProtocolEvidence(): Promise<void> {
  await mkdir(path.dirname(PROTOCOL_LOG_PATH), { recursive: true });
  await writeFile(
    PROTOCOL_LOG_PATH,
    `${protocolEvidence.join('\n')}\n`,
    'utf8',
  );
}
...
test('transformAppServerItem maps camelCase ThreadItem types to snake_case itemType', async () => {
  ...
  recordProtocolEvidence('notification mapping: camelCase ThreadItem types map to frontend item events');
});
...
test('handleAppServerNotification handles agentMessage delta and commandExecution outputDelta', async () => {
  ...
  recordProtocolEvidence('notification mapping: streaming deltas and function call updates reach writer');
});
...
test('writes Codex app-server protocol runtime evidence', async () => {
  assert.ok(
    ...
    'protocol evidence must include mapping, failure recovery, and concurrent ownership assertions',
  );
  await writeProtocolEvidence();
});

test('handleAppServerNotification drops payloads whose threadId does not match expected thread', async () => {
  const { handleAppServerNotification } = await import(
    '../../backend/codex-app-server-runtime.ts'
  );
  ...
  assert.equal(messages.length, 0, 'different thread notification should be ignored');
});
