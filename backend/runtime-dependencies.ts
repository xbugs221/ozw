...
function checkRunnerContract(commandPath) {
  const result = spawnSync(commandPath || RUNNER_COMMAND_NAME, RUNNER_CONTRACT_COMMAND, { encoding: 'utf8' });
  ...
  if (result.error || result.status !== 0) {
    return {
      ...
      error: formatCommandFailure(RUNNER_COMMAND_NAME, RUNNER_CONTRACT_COMMAND, result.error ? result.error.message : output || `exit ${result.status}`),
    };
  }
  let payload;
  try {
    ...
    return {
      ...
      error: formatCommandFailure(RUNNER_COMMAND_NAME, RUNNER_CONTRACT_COMMAND, `invalid JSON: ${error.message}`),
    };
  }
  ...
  return {
    ...
    error: missing.length === 0 ? '' : formatCommandFailure(RUNNER_COMMAND_NAME, RUNNER_CONTRACT_COMMAND, `missing ${missing.join(', ')}`),
  };
}
 ...
 */
function checkGraphCapability(commandPath, contractCapabilities) {
  ...
  const result = spawnSync(commandPath || RUNNER_COMMAND_NAME, graphHelpArgs, { encoding: 'utf8' });
  ...
  if (result.error) {
    return {
      ...
      error: formatCommandFailure(RUNNER_COMMAND_NAME, graphHelpArgs, result.error.message),
...
export function getRuntimeDependencyDiagnostics() {
  ...
  commands.oz.contract = commands.oz.command_path
    ? checkRunnerContract(commands.oz.command_path)
    : { ok: false, required: [`${RUNNER_COMMAND_NAME} ${RUNNER_CONTRACT_COMMAND.join(' ')}`], missing: ['oz'], error: `oz not found in PATH: ${process.env.PATH || ''}` };
  commands.oz.graph = commands.oz.command_path && commands.oz.contract.ok
    ? checkGraphCapability(commands.oz.command_path, commands.oz.contract.capabilities)
