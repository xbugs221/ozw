...
import { reduceNativeRuntimeEvent, ChatMessageLike } from '../frontend/components/chat/utils/nativeRuntimeTranscript.js';
import {
...
import {
  ...
  completeActiveTurnOverlay,
  recordActiveTurnRuntimeEvent,
...
// ---------------------------------------------------------------------------
// Event types forwarded to the frontend
// ---------------------------------------------------------------------------
...
type PiSessionRecord = {
  ...
  /** Snapshot preserved after turn completes, so getNativeSessionLiveTranscript
   *  can serve a bridge until JSONL is ready. */
  lastCompletedLiveMessages: ChatMessageLike[] | null;
  ...
  return {
    send(data: unknown): void {
      ...
      if (message?.type === `${provider}-complete`) {
        completeActiveTurnOverlay(provider, sessionId, projectPath);
      }
      if (message?.type === `${provider}-error` || message?.type === 'session-aborted') {
        clearActiveTurnOverlay(provider, sessionId, projectPath);
      }
      writer.send(data);
    },
...
function runFakePiTurn(session: PiSessionRecord, text: string, runningBehavior: RunningBehavior | undefined, options: {
  ...
  windowlessSetTimeout(async () => {
    ...
    // Populate live transcript so getNativeSessionLiveTranscript can serve
    // messages during streaming (before pi-complete).
    session.liveMessages = reduceNativeRuntimeEvent(
      session.liveMessages,
      { type: 'pi-response', data: { ...finalData }, sessionId: providerSessionId },
    );
    // Keep a snapshot after completion so the API has a bridge before JSONL settles.
    session.lastCompletedLiveMessages = session.liveMessages.length > 0
      ? session.liveMessages.map((m) => ({ ...m }))
      : null;
    session.writer?.send({ type: 'pi-response', data: finalData, sessionId: providerSessionId });
    session.writer?.send({ type: 'pi-complete', sessionId: providerSessionId, actualSessionId: providerSessionId });
    session.writer?.send({
      type: 'session-status',
      sessionId: providerSessionId,
      provider: 'pi',
...
export function seedRunningPiSessionForTest(
  ...
  session.status = 'running';
  session.liveMessages = liveMessages.map((m) => ({ ...m }));
}
...
export function getNativeSessionLiveTranscript(provider: Provider, ozwSessionId: string, projectPath = ''): ChatMessageLike[] | null {
  const session = findRuntimeSession(provider, ozwSessionId, projectPath);
  if (!session) return null;
  // Only running sessions serve their in-progress live transcript.
  // Completed/failed sessions must use the JSONL-based path so the full
  // conversation history (not just the last snapshot) is returned.
  if ((session.status as string) === 'running') {
    const messages = (session as CodexSessionRecord | PiSessionRecord).liveMessages;
    if (messages && messages.length > 0) return messages.map((m) => ({ ...m }));
    return null;
  }
  return null;
}
...
/**
 * Return the last completed live snapshot for a Pi session as a bridge
 * between pi-complete and JSONL being flushed/readable.
 * This is a fallback only — the primary path must always try JSONL first.
 */
export function getPiSessionCompletedSnapshot(ozwSessionId: string, projectPath = ''): ChatMessageLike[] | null {
  const session = findRuntimeSession('pi', ozwSessionId, projectPath);
  if (!session || (session.status as string) === 'running') return null;
  const piSession = session as PiSessionRecord;
  if (piSession.lastCompletedLiveMessages && piSession.lastCompletedLiveMessages.length > 0) {
    return piSession.lastCompletedLiveMessages.map((m) => ({ ...m }));
  }
  return null;
}

/**
 * Clear the last completed live snapshot after a successful JSONL read,
 * preventing stale snapshots from bypassing the full history on subsequent requests.
 */
export function clearPiSessionSnapshot(ozwSessionId: string, projectPath = ''): void {
  const session = findRuntimeSession('pi', ozwSessionId, projectPath);
  if (!session) return;
...
async function ensurePiSession(session: PiSessionRecord, options: {
  ...
  piSession.subscribe((event) => {
    ...
    if (ev.type === 'agent_end') {
      ...
      if (lastAssistant?.content && session.lastMessageId) {
        ...
        if (textParts.length > 0) {
          ...
          session.liveMessages = reduceNativeRuntimeEvent(session.liveMessages, { type: 'pi-response', data: finalData });
        }
      }
      // If no lastMessageId, don't send a duplicate agent_message without id;
      // streaming deltas already built the live transcript, and pi-complete
      // triggers JSONL reload for persisted history.
      // Preserve the last live snapshot as a bridge so getNativeSessionLiveTranscript
      // can serve messages between pi-complete and JSONL being flushed/readable.
      session.lastCompletedLiveMessages = session.liveMessages.length > 0
        ? session.liveMessages.map((m) => ({ ...m }))
        : null;
      session.writer?.send({ type: 'pi-complete', sessionId: piSession.sessionId, actualSessionId: piSession.sessionId });
      session.status = 'completed';
      session.turnStartedAt = null;
      session.liveMessages = [];
      return;
    ...
    if (ev.type === 'message_update') {
      ...
      session.liveMessages = reduceNativeRuntimeEvent(session.liveMessages, { type: 'pi-response', data: t }) as ChatMessageLike[];
      return;
    }
    ...
    if (ev.type === 'tool_execution_start' || ev.type === 'tool_execution_update' || ev.type === 'tool_execution_end') {
      ...
      session.liveMessages = reduceNativeRuntimeEvent(session.liveMessages, { type: 'pi-response', data: transformed });
      return;
    }
    ...
    if (ev.type === 'error') {
      getPiStreamingDeltaBatcher(session).flushSession(piSession.sessionId);
      session.writer?.send({ type: 'pi-error', error: String(ev.message || 'Pi error'), sessionId: piSession.sessionId });
...
async function runPiTurn(session: PiSessionRecord, text: string, runningBehavior: RunningBehavior | undefined, options: {
  ...
  void piSession.prompt(text, promptOptions).catch((error: unknown) => {
    ...
    session.writer?.send({ type: 'pi-error', error: err?.message || 'Pi error', sessionId: piSession.sessionId });
  });
}
...
export async function sendNativeMessage(input: {
  ...
  if (provider === 'pi') {
    ...
    try {
      ...
      session.status = 'failed';
      const err = error as { message?: string };
      session.writer?.send({ type: 'pi-error', error: err?.message || 'Pi session init failed', sessionId });
