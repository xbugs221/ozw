...
| JSONL Render 快照按视口渐进加载 | 长会话先展示当前页和一页预留，进入预留区才读取旧历史 | `tests/specs/chat-performance-boundary.spec.ts` | 真实聊天入口源码和渲染快照状态 | `ChatInterface`、`renderSnapshotController` | 请求上限 50；折叠布局按实际视口收敛；独立滚动控制器前插一页并恢复锚点；不默认后台扫描 | 极端字体缩放由浏览器验收覆盖 |
...
- 并且每个原始消息请求的 `limit` 不得超过 50
- 并且折叠后的首屏只挂载当前页，首屏绘制后可静默预热约六屏旧历史数据
- 当用户进入预留页时，rendered snapshot 必须使用独立分页状态读取下一页旧历史
- 并且进入预留页时必须先消费缓冲、一次补充约三屏，并按稳定消息锚点恢复位置
- 并且预热最多连续读取三批，不能退化成完整历史 hydration 循环
...
- 覆盖核心业务契约：Render 快照按视口收敛首屏、请求保持有界、进入预留区才分页，且使用独立锚点恢复而不触发主会话状态。
