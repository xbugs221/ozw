...
- **且** 请求必须继续携带有界对话分页参数，如 `limit=50` 和后续 cursor/offset
- **且** 折叠思考与工具细节可随所属对话页返回，但不能单独消耗该页配额
...
- **且** 不得请求高 offset 历史页
- **当** 首屏内容已经可见
- **则** 页面可以在首屏绘制完成后静默预热最多三批旧历史
- **且** 预热数据只进入内存缓冲，不得扩大首屏 DOM 或阻塞当前交互
- **且** 页面不得串行扫描完整旧历史

### 场景：Render 按实际视口维持多屏预留

- **给定** Render 已取得一个不超过 50 条对话内容配额的尾部批次
- **当** 折叠后的展示块完成布局
- **则** 首屏滚动高度应保持约两屏，内存中应准备约六屏旧历史
- **当** 用户进入该预留页
- **则** Render 必须先消费已有内存缓冲，不得等待新的历史请求
- **且** 一次用户触发应补充约三屏可滚动历史，并按稳定消息锚点恢复位置
- **且** 缓冲低于目标水位时才能继续有界预热
