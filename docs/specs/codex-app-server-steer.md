...
### 需求：Codex 生产执行只能进入 app-server runtime

Codex 任务不得通过 `backend/openai-codex.ts`、`queryCodex` 或 `codex exec --json` 单次执行路径运行。所有生产入口必须进入 Codex app-server runtime，从而继承 thread/turn、active turn、live transcript 和 `turn/steer` 能力。

#### 场景：`/api/agent` 不再调用单次 Codex exec

- **给定** 用户通过 `POST /api/agent` 触发 Codex 任务
- **当** 后端解析 route 并启动 Codex runtime
- **则** 生产源码不得存在 `backend/openai-codex.ts`
- **且** route 不得 import 旧 `openai-codex` 或调用 `queryCodex`
- **且** 生产源码不得构造 `codex exec --json`
- **且** route 必须接入 Codex app-server runtime 或专用 app-server runner

#### 场景：OpenAI 语音转写和 GPT 增强直连被移除

- **给定** 用户使用附件上传、麦克风或相关前端入口
- **当** 前后端处理这些请求
- **则** 生产源码不得调用 OpenAI Whisper/GPT HTTP API
- **且** 生产源码不得静态或动态导入 `openai` npm SDK
- **且** `package.json` 不得直接声明 `openai` 依赖
- **且** 前端不得继续调用 `/api/transcribe-audio`

#### 场景：活跃规格不再要求旧单次路径

- **给定** 审阅者检查当前 specs 和规格测试
- **当** 执行 provider runtime 边界规格测试
- **则** 活跃规格不得把 `backend/openai-codex.ts`、`queryCodex` 或 `codex exec --json` 表达为当前要求

对应规格测试：`tests/specs/provider-runtime-boundary.spec.ts`，并生成 `test-results/provider-runtime/source-audit.json`。
...
- **给定** 同一用户在窗口 B 订阅了 `c2`
- **当** 窗口 A 的 `c1` 会话产生 Codex/Pi 私有 delta
- **则** 窗口 B 不得仅因同用户、同 provider 而收到 `c1` 消息
- **且** 后端必须用 `sessionId`、`ozwSessionId`、`providerSessionId` 或 `clientRequestId` 等稳定会话身份至少一项相等来判定订阅匹配
...
- [Codex app-server 历史、顺序和恢复](./codex-app-server-history.md)
