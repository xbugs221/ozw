...
| --- | --- | --- |
| 真实 server 测试通过共享 helper 启动 | `backend-service-fixture-contract` | `backend-service-fixture-source` |
| WebSocket 测试统一使用 Authorization header | `backend-service-fixture-contract` | `backend-service-fixture-source` |
| 子进程环境统一隔离数据库和 JWT 密钥 | `backend-service-fixture-contract` | `backend-service-fixture-source` |

### 需求：真实后端服务测试必须复用共享夹具

#### 场景：真实 server 测试通过共享 helper 启动

- **测试文件**：`docs/changes/2-统一后端真实服务测试夹具/tests/backend-service-fixture.contract.test.ts`
...
- **剩余风险**：不强制迁移所有后端测试，只迁移真实 server fixture 重复最明显的测试
